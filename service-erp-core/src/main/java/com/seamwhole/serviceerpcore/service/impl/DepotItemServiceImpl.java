...
public class DepotItemServiceImpl implements DepotItemService {
    ...
    public List<DepotItem> select(String name, Integer type, String remark, int offset, int rows) {
        return depotItemExtMapper.selectByConditionDepotItem(name, type, remark, offset, rows);
    }

    public Long countDepotItem(String name, Integer type, String remark) {
        return depotItemExtMapper.countsByDepotItem(name, type, remark);
    }
    ...
    public List<DepotItemVo4HeaderId> getHeaderIdByMaterial(String materialParam, String depotIds) {
        return depotItemExtMapper.getHeaderIdByMaterial(materialParam, depotIds);
    }
    ...
    public List<DepotItemVo4DetailByTypeAndMId> findDetailByTypeAndMaterialIdList(Map<String, String> map) {
        ...
        return depotItemExtMapper.findDetailByTypeAndMaterialIdList(mId, QueryUtils.offset(map), QueryUtils.rows(map));
    }
    ...
    public Long findDetailByTypeAndMaterialIdCounts(Map<String, String> map) {
        ...
        return depotItemExtMapper.findDetailByTypeAndMaterialIdCounts(mId);
    }
    ...
    public List<DepotItemVo4Material> findStockNumByMaterialIdList(Map<String, String> map) {
        ...
        return depotItemExtMapper.findStockNumByMaterialIdList(mId, monthTime, QueryUtils.offset(map), QueryUtils.rows(map));
    }
    ...
    public Long findStockNumByMaterialIdCounts(Map<String, String> map) {
        ...
        String monthTime = map.get("monthTime");
        return depotItemExtMapper.findStockNumByMaterialIdCounts(mId, monthTime);
    }
    ...
    public int findByTypeAndMaterialId(String type, Long mId) {
        if(type.equals(TYPE)) {
            return depotItemExtMapper.findByTypeAndMaterialIdIn(mId);
        } else {
            return depotItemExtMapper.findByTypeAndMaterialIdOut(mId);
        }
    }

    public List<DepotItemVo4WithInfoEx> getDetailList(Long headerId) {
        return depotItemExtMapper.getDetailList(headerId);
    }

    public List<DepotItemVo4WithInfoEx> findByAll(String headIds, String materialIds, Integer offset, Integer rows) {
        return depotItemExtMapper.findByAll(headIds, materialIds, offset, rows);
    }

    public int findByAllCount(String headIds, String materialIds) {
        return depotItemExtMapper.findByAllCount(headIds, materialIds);
    }

    public BigDecimal findByType(String type, Integer ProjectId, Long MId, String MonthTime, Boolean isPrev) {
        if (TYPE.equals(type)) {
            if (isPrev) {
                return depotItemExtMapper.findByTypeInIsPrev(ProjectId, MId, MonthTime);
            } else {
                return depotItemExtMapper.findByTypeInIsNotPrev(ProjectId, MId, MonthTime);
            }
        } else {
            if (isPrev) {
                return depotItemExtMapper.findByTypeOutIsPrev(ProjectId, MId, MonthTime);
            } else {
                return depotItemExtMapper.findByTypeOutIsNotPrev(ProjectId, MId, MonthTime);
            }
        }
    }
    ...
    public BigDecimal findPriceByType(String type, Integer ProjectId, Long MId, String MonthTime, Boolean isPrev) {
        if (TYPE.equals(type)) {
            if (isPrev) {
                return depotItemExtMapper.findPriceByTypeInIsPrev(ProjectId, MId, MonthTime);
            } else {
                return depotItemExtMapper.findPriceByTypeInIsNotPrev(ProjectId, MId, MonthTime);
            }
        } else {
            if (isPrev) {
                return depotItemExtMapper.findPriceByTypeOutIsPrev(ProjectId, MId, MonthTime);
            } else {
                return depotItemExtMapper.findPriceByTypeOutIsNotPrev(ProjectId, MId, MonthTime);
            }
        }
    }
    ...
    public BigDecimal findAssembleByType(String subType, String mType, Integer ProjectId, Long MId, String MonthTime, Boolean isPrev) {
        if (isPrev) {
            return depotItemExtMapper.findAssembleIsPrev(subType, mType, ProjectId, MId, MonthTime);
        } else {
            return depotItemExtMapper.findAssembleIsNotPrev(subType, mType, ProjectId, MId, MonthTime);
        }
    }

    public BigDecimal buyOrSale(String type, String subType, Long MId, String MonthTime, String sumType) {
        if (SUM_TYPE.equals(sumType)) {
            return depotItemExtMapper.buyOrSaleNumber(type, subType, MId, MonthTime, sumType);
        } else {
            return depotItemExtMapper.buyOrSalePrice(type, subType, MId, MonthTime, sumType);
        }
    }
                ...
                for (int i = 0; i < deletedJson.size(); i++) {
                    ...
                    if(BusinessConstants.DEPOTHEAD_TYPE_OUT.equals(depotHead.getType())
                        ...
                        if(BusinessConstants.ENABLE_SERIAL_NUMBER_ENABLED.equals(material.getEnableserialnumber())){
                            serialNumberExtMapper.cancelSerialNumber(depotItem.getMaterialid(),depotItem.getHeaderid(),(depotItem.getBasicnumber()==null?0:depotItem.getBasicnumber()).intValue(),
...
//                            String UnitName = findUnitName(mId); //查询计量单位名称
    ...
    public int batchDeleteDepotItemByIds(String ids) {
        ...
        return depotItemExtMapper.batchDeleteDepotItemByIds(new Date(),userInfo==null?null:userInfo.getId(),idArray);
    }
}
