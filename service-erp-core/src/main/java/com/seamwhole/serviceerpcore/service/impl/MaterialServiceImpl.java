...
public class MaterialServiceImpl implements MaterialService {
    ...
    public List<MaterialVo4Unit> select(String name, String model, Long categoryId, String categoryIds, String mpList, int offset, int rows) {
        ...
        List<MaterialVo4Unit> list = materialExtMapper.selectByConditionMaterial(name, model,categoryId,categoryIds,mpList, offset, rows);
        if (null != list) {
    ...
    public Long countMaterial(String name, String model,Long categoryId, String categoryIds,String mpList) {
        return materialExtMapper.countsByMaterial(name, model,categoryId,categoryIds,mpList);
    }
    ...
    public int updateMaterial(String beanJson, Long id) {
        ...
        if(unitId != null) {
            materialExtMapper.updatePriceNullByPrimaryKey(id); //将价格置空
        } else {
            materialExtMapper.updateUnitIdNullByPrimaryKey(id); //将多单位置空
        }
        return res;
    }
    ...
    public String findUnitName(Long mId){
        return materialExtMapper.findUnitName(mId);
    }

    public List<MaterialVo4Unit> findById(Long id){
        return materialExtMapper.findById(id);
    }

    public List<MaterialVo4Unit> findBySelect(){
        return materialExtMapper.findBySelect();
    }
    ...
    public List<MaterialVo4Unit> findByAll(String name, String model, Long categoryId, String categoryIds) {
        List<MaterialVo4Unit> resList = new ArrayList<MaterialVo4Unit>();
        List<MaterialVo4Unit> list = materialExtMapper.findByAll(name, model, categoryId, categoryIds);
        if (null != list) {
            for (MaterialVo4Unit m : list) {
                resList.add(m);
            }
    ...
    public List<Material> getMaterialEnableSerialNumberList(Map<String, Object> parameterMap) {
        return materialExtMapper.getMaterialEnableSerialNumberList(parameterMap);
    }
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public int batchDeleteMaterialByIds(String ids) {
        ...
        return materialExtMapper.batchDeleteMaterialByIds(new Date(),userInfo==null?null:userInfo.getId(),idArray);
    }
    /**
    ...
    public int batchDeleteMaterialByIdsNormal(String ids) throws Exception{
         ...
         * */
        List<DepotItem> depotItemList=depotItemExtMapper.getDepotItemListListByMaterialIds(idArray);
